...
class boss_emalon : public CreatureScript
{
    public:
        ...
        struct boss_emalonAI : public BossAI
        {
			...
			{
				if(instance)
					instance->DoCompleteAchievement(RAID_MODE(3136,3137));
            ...
            void EnterCombat(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_LIGHTNING_NOVA, 18000);
                events.ScheduleEvent(EVENT_BERSERK, 360000);
                events.ScheduleEvent(EVENT_OVERCHARGE, 22000);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHAIN_LIGHTNING:
                            ...
                            events.ScheduleEvent(EVENT_CHAIN_LIGHTNING, urand(8000,12000));
                            break;
                        ...
                        case EVENT_OVERCHARGE:
                            if (!summons.empty())
                            {
                                ...
                                if (minion && minion->IsAlive())
                                {
                                    ...
                                    events.ScheduleEvent(EVENT_OVERCHARGE, 26000);
                                }
                            }
                            break;
                        case EVENT_BERSERK:
...
class mob_tempest_minion : public CreatureScript
{
    public:
        ...
        struct mob_tempest_minionAI : public ScriptedAI
        {
            ...
            {
                DoZoneInCombat();
                events.ScheduleEvent(EVENT_SHOCK, urand(4000,12000));
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (events.ExecuteEvent() == EVENT_SHOCK)
                {
                    ...
                    events.ScheduleEvent(EVENT_SHOCK, urand(6000,7500));
                }
