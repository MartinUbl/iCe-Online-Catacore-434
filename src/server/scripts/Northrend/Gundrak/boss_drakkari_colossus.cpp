...
public:
    ...
    struct boss_drakkari_colossusAI : public ScriptedAI
    {
        ...
        void CreatureState(Creature* pWho, bool bRestore = false)
        {
            ...
            if (bRestore)
            {
                pWho->ClearUnitState(UNIT_STATE_STUNNED | UNIT_STATE_ROOT);
            ...
            }else
            {
                ...
                pWho->AddUnitState(UNIT_STATE_STUNNED | UNIT_STATE_ROOT);
                if (pWho == me)
...
public:
    ...
    struct boss_drakkari_elementalAI : public ScriptedAI
    {
        ...
        uint32 uiSurgeTimer;
		uint32 uiPostMergeTimer;
		uint32 uiInitSummonTimer;
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!bGoToColossus && HealthBelowPct(50))
            {
                if (Creature *pColossus = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_DRAKKARI_COLOSSUS) : 0))
                {
                    if (!CAST_AI(boss_drakkari_colossus::boss_drakkari_colossusAI,pColossus->AI())->HealthBelowPct(10))
                    {
                        me->InterruptNonMeleeSpells(true);
                        DoCast(pColossus, SPELL_MERGE);
						uiPostMergeTimer = 2500;
                        bGoToColossus = true;
                    }
                }
            }
			...
			if(uiPostMergeTimer)
			{
				if(uiPostMergeTimer <= diff)
				{
		            ...
		            me->RemoveFromWorld();
					uiPostMergeTimer = 0;
				} else uiPostMergeTimer -= diff;
			}

			if(uiInitSummonTimer)
			{
