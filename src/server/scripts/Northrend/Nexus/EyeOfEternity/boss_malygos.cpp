...
enum
{
    ...
    SAY_AGGRO3                     = -1616019,
    SAY_VORTEX                     = -1616006,
    SAY_POWER_SPARK                = -1616035, //missing in DB
...
public:
    ...
    struct boss_malygosAI : public ScriptedAI
    {
        boss_malygosAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            ...
            //iCelike - reduce Vortex damage to 1500
            if(SpellEntry* pVortexDmg = (SpellEntry*)sSpellStore.LookupEntry(56256))
                pVortexDmg->EffectBasePoints[1] = 1499;
        }
     ...
     //   uint32 m_uiCheckDisksTimer;
		...
		bool m_bNeedTarget; // OJaaaa
        
        void Reset()
        {
            if(m_pInstance)
                m_pInstance->SetData(TYPE_MALYGOS, NOT_STARTED); //also respawn GOs
            else
                me->ForcedDespawn();
       ...
       //     m_uiCheckDisksTimer = 2500;
        ...
        {
			if(m_uiPhase == PHASE_NOSTART) //OJaaaa
			{
				if(pWho && ((Player *)pWho)->HasItemCount(m_uiIs10Man ? KEY : KEY_H, 1, false))
				{
        ...
        void DamageTaken(Unit* pDoneBy, uint32 &uiDamage)
        {
			...
			if(m_uiPhase == PHASE_DRAGONS) //multiply damage by a coeficient substituting vehicle spell power. OJaaaa
			{
				uiDamage = uint32((float)uiDamage * 1.2f); //todo: find an appropriate value
			}
        }
        ...
        void KilledUnit(Unit* pVictim)
        {
            ...
            {
                if(pVictim && pVictim->GetEntry() == NPC_WYRMREST_SKYTALON)
                {
                    for(std::list<std::pair<uint64, uint64> >::iterator iter = m_uiMounts.begin(); iter != m_uiMounts.end(); ++iter)
                    {
                        if(pVictim->GetGUID() == (*iter).first)
                        {
							...
							((Creature *)pVictim)->SetVisible(false);
                            return;
                        }
                    }
                }
        ...
        void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
        {
			...
			if(pSpell->Id == SPELL_BERSERK) //OJaaaa
			{
   ...
   /*         if(m_uiPhase == PHASE_ADDS)
            ...
            if(m_uiPhase == PHASE_FLOOR)
            {
                ...
                {
                    PowerSpark(1);
                    DoScriptText(SAY_POWER_SPARK, me); //not in DB
                    m_uiPowerSparkTimer = 30000;
                ...
                if(m_uiTimer<= uiDiff)
                {
					if(me->HasAura(SPELL_BERSERK)) //OJaaaa. not to enter phase 2 while enraged
					{
            ...
            else if(m_uiPhase == PHASE_DRAGONS)
            {
                ...
                else if(m_uiSubPhase == SUBPHASE_DESTROY_PLATFORM2)
                {
                    if(m_uiTimer <= uiDiff)
                    {
                        ...
                        if (Creature* m_uiSafeZone = me->SummonCreature(30494, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 360000)) //safe zone.. raid do not leave combat while mounting
                        {
                            ...
                            m_uiSafeZone->SetDisplayId(17612); // previously 11686, new one hoped to be invisible - OJaaaa
                        }
                        //Prepare Mounts
                        PrepareMounts();
                ...
                }else if(m_uiSubPhase == SUBPHASE_DESTROY_PLATFORM3)
                {
                    if(m_uiTimer<= uiDiff)
                    {
                        ...
                        if(Unit *pVehicle = ((Unit*)Unit::GetUnit(*me, me->getVictim()->GetGUID())))
                        {
							...
							if(me->HasAura(SPELL_BERSERK)) //OJaaaa. this is there just for case of enrage
							{
								SetCombatMovement(true);
								me->GetMotionMaster()->MoveChase(pVehicle);
							}
                ...
                if(m_uiSurgeOfPowerTimer <= uiDiff)
                {
                    for(int i = 0; i < 1; ++i)
                    {
                        ...
                        if(i == 0)
                        {
                            me->CastSpell(pTarget, /*m_uiIs10Man ?*/ SPELL_SURGE_OF_POWER /*: SPELL_SURGE_OF_POWER_H*/, false);
            ...
            else if(m_uiPhase == PHASE_OUTRO)
            {
                if(m_uiSubPhase == SUBPHASE_STOP_COMBAT)
                {
					...
					me->SetFlying(false); //OJaaaa
					me->SetFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_DEAD); //feign death
                ...
                if(m_uiSpeechTimer[m_uiSpeechCount] <= uiDiff)
                {
                    ...
                    switch(m_uiSpeechCount)
                    {
                        ...
                        case 4:
                            ...
                            //summon loot
							if(GameObject *pGift = pAlexstrasza->SummonGameObject( m_uiIs10Man ? GO_ALEXSTRASZAS_GIFT : GO_ALEXSTRASZAS_GIFT_H, 729.50f, 1309.50f, 274.00f, GOPositions[2].o, 0, 0, 0, 0, 0))
                                pAlexstrasza->SetFacingToObject(pGift);

                            //Summon platform for the looters to stay on...
							//pAlexstrasza->SummonGameObject( 190023 , 757.001f, 1297.4f, 267.09f, 2.26684f, 0.0f, 0.0f, 0.905866f, 0.423563f, 600000);
							pAlexstrasza->SummonGameObject( 590011 , 730.684f, 1327.92f, 266.234f, -0.802851f, 0.0f, 0.0f, 0.905866f, 0.423563f, 600000);

							if(m_pInstance)//OJaaaa
							{
							    Map* pMap = me->GetMap();
							    if(!pMap)
								{
...
public:
    ...
    struct mob_power_sparkAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* pDoneBy, uint32 &uiDamage)
        {
			if (pDoneBy && ((Creature *)pDoneBy)->GetEntry() == NPC_MALYGOS) //OJaaaa. not to kill the spark by boss
			{
				uiDamage = 0;
				return;
			}
...
public:
    ...
    struct mob_scion_of_eternityAI : public ScriptedAI
    {
		...
		// workaround. disks dont work. need to force scions to land one by one.
		void JustDied(Unit* pKiller) //OJaaaa
		{
			if(Creature* pCreature = GetClosestCreatureWithEntry(me, NPC_SCION_OF_ETERNITY, 100.0f, true))
				((mob_scion_of_eternityAI *)pCreature->AI())->WorkaroundAttack(pKiller);
		}
...
public:
    ...
    struct mob_nexus_lordAI : public ScriptedAI
    {
		...
		void JustDied(Unit* pKiller) //OJaaaa
		{
			if(GetClosestCreatureWithEntry(me, NPC_NEXUS_LORD, 100.0f, true))
				return;

			if(Creature* pCreature = GetClosestCreatureWithEntry(me, NPC_SCION_OF_ETERNITY, 100.0f, true))
				me->DealDamage(pCreature, 1); // :P
		}
...
class go_focusing_iris : public GameObjectScript //OJaaaa
{
public:
    ...
    {
		if(Creature* pBoss = GetClosestCreatureWithEntry(pGO, NPC_MALYGOS, 200.0f, true))
		{
			if(pPlayer && pPlayer->HasItemCount(((ScriptedAI *)pBoss->AI())->RAID_MODE(true, false) ? KEY : KEY_H, 1, false))
			{
				pPlayer->CastSpell(pGO->GetPositionX(), pGO->GetPositionY(), pGO->GetPositionZ(), 61012, true); // visual effect
				pBoss->AI()->EnterCombat(pPlayer); // :P
				return true;
			}
		}
		return false;
