...
public:
    ...
    class spell_gen_leeching_swarm_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const * aurEff)
        {
            ...
            {
                int32 lifeLeeched = int(pTarget->GetHealth() * aurEff->GetAmount() / 100);
                if (lifeLeeched < 250)
                    lifeLeeched = 250;
                // Damage
            ...
            void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
            {
                ...
                switch (GetSpellProto()->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_PRIEST:
                        heal = 1.885f * float(GetCaster()->SpellBaseDamageBonusDone(GetSpellSchoolMask(GetSpellProto())));
                        break;
                    case SPELLFAMILY_PALADIN:
                    case SPELLFAMILY_SHAMAN:
                        heal = std::max(1.885f * float(GetCaster()->SpellBaseDamageBonusDone(GetSpellSchoolMask(GetSpellProto()))), 1.1f * float(GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK)));
                        break;
