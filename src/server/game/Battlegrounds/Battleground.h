...
enum BattlegroundSpells
{
    ...
    SPELL_FACTION_HORDE             = 88172,                // Faction Override -> Horde
    SPELL_FACTION_ALLIANCE          = 88174                 // Faction Override -> Alliance
};
...
class Battleground
{
    ...
    public:
        ...
        void SetWargame(bool state)         { m_IsWargame = state; }
        ...
        bool AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime = 0, uint32 animProgress = 100, uint32 createState = 1 /* GO_STATE_READY */);
