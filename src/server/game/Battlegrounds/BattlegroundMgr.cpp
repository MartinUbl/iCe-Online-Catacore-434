    ...
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:
        {
            ...
            *data << uint32(Time2);                     // Elapsed Time
            ...
            // following fields also controls messages "Not enough players. The game will end in .. Min" in BGs
            if (bg->isArena())
...
{
    uint8 isRated = (bg->isRated() ? 1 : 0);               // is rated? (normal = 0 / rated = 1)
    uint8 isArena = (bg->isArena() ? 1 : 0);               // is arena? (bg = 0 / arena = 1)
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = bg->GetPlayerScoresBegin(); itr != bg->GetPlayerScoresEnd(); itr++)
    {
        ...
        {
            sLog->outError("Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", playerGuid, bg->GetTypeID(true));
            continue;
        }
        ...
        data->WriteBit(unk1);       // unk1 (binary)
        data->WriteBit(unk2);       // hasUnk2
        data->WriteBit(guid[2]);
        data->WriteBit(!isArena);   // is battleground?
        data->WriteBit(unk4);       // hasUnk4
        data->WriteBit(isRated);    // is rated? - send personal rating change
        data->WriteBit(unk6);       // hasUnk6
        data->WriteBit(guid[3]);
        data->WriteBit(guid[0]);
        data->WriteBit(guid[5]);
        data->WriteBit(guid[1]);
        data->WriteBit(guid[6]);
        data->WriteBit(player->GetBGTeam() == ALLIANCE);// controls name color (blue, red)
        ...
        if (isRated)
        {
            ...
            buff << int32(score->PersonalRatingChange);      // personal rating change
        ...
        switch (bg->GetTypeID(true))                             // Custom values
        {
            case BATTLEGROUND_RB:
            {
                switch (bg->GetMapId())
                {
                    ...
                    case 529:
                        data->WriteBits(0x00000002, 24);
                        buff << uint32(((BattlegroundABScore*)score)->BasesAssaulted);      // bases asssulted
                    ...
                    case 628:                                   // IC
                        data->WriteBits(0x00000002, 24);
                        buff << uint32(((BattlegroundICScore*)score)->BasesAssaulted);       // bases asssulted
                    ...
                    case 761:
                        data->WriteBits(0x00000002, 24);
                        buff << uint32(((BattlegroundBGScore*)score)->BasesAssaulted);      // bases asssulted
                        buff << uint32(((BattlegroundBGScore*)score)->BasesDefended);       // bases defended
                        break;
                    default:
                        data->WriteBits(0, 24);
                        break;
    ...
    data->WriteBit(bg->GetStatus() == STATUS_WAIT_LEAVE);    // If Ended (has winner?)
    data->FlushBits();
    data->PutBits<int32>(count_pos, count, 21);              // Number of Players

    if (isRated)
    {
        *data << uint32(bg->m_ArenaTeamMMR[playerTeam]);    // prematch MMR?
        *data << uint32(0);                                 // prematch?
        *data << uint32(0);                                 // postmatch?
...
void BattlegroundMgr::UpdateArenaQueueInfo()
{
    // set zeros to whole array
    memset(m_teamsInQueue, 0, sizeof(uint32) * ARENA_SLOT_MAX);

    // count teams queued in all battleground queues
    for (uint32 queueId = BATTLEGROUND_QUEUE_2v2; queueId <= BATTLEGROUND_QUEUE_5v5; queueId++)
    {
        // retrieve queue
        BattlegroundQueue& bgQueue = m_BattlegroundQueues[queueId][BATTLEGROUND_NOTWINK];

        // we need to go through all level brackets
        for (int bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
        {
            for (int queueGroupType = BG_QUEUE_PREMADE_ALLIANCE; queueGroupType < BG_QUEUE_GROUP_TYPES_COUNT; ++queueGroupType)
                m_teamsInQueue[queueId - BATTLEGROUND_QUEUE_2v2] += bgQueue.m_QueuedGroups[bracket][BG_QUEUE_NORMAL_ALLIANCE].size() + bgQueue.m_QueuedGroups[bracket][BG_QUEUE_NORMAL_HORDE].size();
        }

        uint32 divisor = 2;
        if (queueId == BATTLEGROUND_QUEUE_3v3)
            divisor = 3;
        else if (queueId == BATTLEGROUND_QUEUE_5v5)
            divisor = 5;

        m_teamsInQueue[queueId - BATTLEGROUND_QUEUE_2v2] += bgQueue.m_QueuedPlayers.size() / divisor;
    }
    ...
    // set zeros to whole array
    memset(m_teamsInArena, 0, sizeof(uint32) * ARENA_SLOT_MAX);
    ArenaSlot sl;
    ...
    for (BattlegroundSet::iterator itr = tset.begin(); itr != tset.end(); ++itr)
    {
        ...
        sl = ArenaSlotForType(itr->second->GetArenaType());
        if (sl == ARENA_SLOT_NONE)
            continue;
...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket *data, const uint64& guid, Player* plr, BattlegroundTypeId bgTypeId)
{
    ...
    *data << uint8(0);                                      // min level
    ...
    data->WriteBit(1);                                      // hide battleground window
