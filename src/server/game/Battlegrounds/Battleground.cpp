        ...
        if (isArena())
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                if (Player *plr = sObjectMgr->GetPlayer(itr->first))
                {
                    ...
                    sBattlegroundMgr->BuildBattlegroundStatusPacket(&status, this, plr, queueSlot, STATUS_IN_PROGRESS, plr->GetBattlegroundQueueJoinTime(m_TypeID), GetElapsedTime(), GetArenaType());
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, plr, plr->GetBattlegroundQueueIndex(bgQueueTypeId), STATUS_IN_PROGRESS, plr->GetBattlegroundQueueJoinTime(GetTypeID()), GetElapsedTime(), GetArenaType());
        plr->GetSession()->SendPacket(&data);
...
void Battleground::AddPlayer(Player* plr)
{
    ...
    // Toggle PvP combat on when entering battleground
    plr->SetPvP(true);
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, uint32 animProgress, uint32 createState)
{
    // If the assert is called, means that m_BgObjects must be resized!
    ASSERT(type < m_BgObjects.size());

    if (createState > GO_STATE_ACTIVE_ALTERNATIVE)
        createState = GO_STATE_READY;
    ...
    if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry, GetBgMap(),
        PHASEMASK_NORMAL, x, y, z, o, rotation0, rotation1, rotation2, rotation3, animProgress, (GOState)createState))
    {
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    if (Creature* pCreature = AddCreature(entry, type, team, x, y, z, o))
    {
        ...
        pCreature->SetFloatValue(UNIT_MOD_CAST_HASTE, 1.0f);
...
void Battleground::RatedBattlegroundWon(Player *player)
{
    ...
    player->ModifyCurrency(CURRENCY_TYPE_CONQUEST_POINTS, CONQUEST_REWARD_RATED_BATTLEGROUND, CURRENCY_SOURCE_BG);
