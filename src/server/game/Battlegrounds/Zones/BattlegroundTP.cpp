...
void BattlegroundTP::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        {
            ++m_minutesElapsed;
            UpdateWorldState(TP_TIME_VALUE, 27 - m_minutesElapsed);
        }
        ...
        if (m_BothFlagsKept)
        {
            ...
            case 9:
            {
                ...
                {
                    if (Player * plr = sObjectMgr->GetPlayer(m_FlagKeepers[0]))
                        plr->CastSpell(plr, m_FlagDebuffState < 4 ? TP_SPELL_FOCUSED_ASSAULT : TP_SPELL_BRUTAL_ASSAULT, true);
                    if (Player * plr = sObjectMgr->GetPlayer(m_FlagKeepers[1]))
                        plr->CastSpell(plr, m_FlagDebuffState < 4 ? TP_SPELL_FOCUSED_ASSAULT : TP_SPELL_BRUTAL_ASSAULT, true);
        ...
        else
        {
            ...
            {
                plr->RemoveAurasDueToSpell(TP_SPELL_FOCUSED_ASSAULT);
                plr->RemoveAurasDueToSpell(TP_SPELL_BRUTAL_ASSAULT);
            }
            if (Player * plr = sObjectMgr->GetPlayer(m_FlagKeepers[1]))
            {
                plr->RemoveAurasDueToSpell(TP_SPELL_FOCUSED_ASSAULT);
...
void BattlegroundTP::EventPlayerCapturedFlag(Player *Source)
{
    ...
    RewardHonorToTeam(BG_TP_Honor[BG_TP_FLAG_CAP], Source->GetBGTeam());
...
void BattlegroundTP::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
        ...
        else
        {
            ...
            if (m_FlagDebuffState >= 1)
            {
                for (uint8 i = 0; i < m_FlagDebuffState; i++)
                    Source->CastSpell(Source, m_FlagDebuffState <= 4 ? TP_SPELL_FOCUSED_ASSAULT : TP_SPELL_BRUTAL_ASSAULT, true);
            }
        }
    ...
    else if (GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10))
    {
...
bool BattlegroundTP::SetupBattleground()
{
    ...
    if (!AddObject(BG_TP_OBJECT_A_FLAG, BG_OBJECT_A_FLAG_TP_ENTRY, 2118.3635f, 191.5493f, 44.0522f, 5.8198f, 0, 0, 0.229627f, -0.973279f, BG_TP_FLAG_RESPAWN_TIME/1000)
        ...
        || !AddObject(BG_TP_OBJECT_DOOR_A_2, BG_OBJECT_DOOR_A_2_TP_ENTRY, 2156.2361f, 219.833f,  43.4905f, 5.72411f, 0, 0, 0.27591f, -0.961184f, RESPAWN_IMMEDIATELY)
    ...
    const uint32 toLockObjects[] = { BG_TP_OBJECT_DOOR_A_1, BG_TP_OBJECT_DOOR_A_2, BG_TP_OBJECT_DOOR_A_3, BG_TP_OBJECT_DOOR_H_1, BG_TP_OBJECT_DOOR_H_2, BG_TP_OBJECT_DOOR_H_3 };
    GameObject* go;
