...
{
    if (nodePoint[NODE_TYPE_QUARRY].nodeState == (player->GetBGTeam() == ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
        player->CastSpell(player, SPELL_QUARRY, true);

    if (nodePoint[NODE_TYPE_REFINERY].nodeState == (player->GetBGTeam() == ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
...
void BattlegroundIC::DoAction(uint32 action, uint64 var)
{
    ...
    plr->SetTransport(plr->GetBGTeam() == ALLIANCE ? gunshipAlliance : gunshipHorde);
    ...
    plr->m_movementInfo.t_guid = (plr->GetBGTeam() == ALLIANCE ? gunshipAlliance : gunshipHorde)->GetGUID();
...
void BattlegroundIC::AddPlayer(Player *plr)
{
    ...
    if (nodePoint[NODE_TYPE_QUARRY].nodeState == (plr->GetBGTeam() == ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
        plr->CastSpell(plr,SPELL_QUARRY,true);

    if (nodePoint[NODE_TYPE_REFINERY].nodeState == (plr->GetBGTeam() == ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
...
void BattlegroundIC::UpdatePlayerScore(Player* Source, uint32 type, int32 value, bool doAddHonor)
{
...
void BattlegroundIC::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    factionReinforcements[GetTeamIndexByTeamId(player->GetBGTeam())] -= 1;

    UpdateWorldState((player->GetBGTeam() == ALLIANCE ? BG_IC_ALLIANCE_RENFORT : BG_IC_HORDE_RENFORT), factionReinforcements[GetTeamIndexByTeamId(player->GetBGTeam())]);

    // we must end the battleground
    if (factionReinforcements[GetTeamIndexByTeamId(player->GetBGTeam())] < 1)
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; i++)
    {
        if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
        {
            ...
            if (nodePoint[i].faction == GetTeamIndexByTeamId(player->GetBGTeam()))
                return;

            uint32 nextBanner = GetNextBanner(&nodePoint[i], GetTeamIndexByTeamId(player->GetBGTeam()), false);

            // we set the new settings of the nodePoint
            nodePoint[i].faction = GetTeamIndexByTeamId(player->GetBGTeam());
            nodePoint[i].last_entry = nodePoint[i].gameobject_entry;
            nodePoint[i].gameobject_entry = nextBanner;
                ...
                nodePoint[i].banners[BANNER_H_CONTESTED] == nextBanner)
            {
                ...
                SendMessage2ToAll(LANG_BG_IC_TEAM_ASSAULTED_NODE_2, CHAT_MSG_BG_SYSTEM_NEUTRAL, player, nodePoint[i].string, (player->GetBGTeam() == ALLIANCE ? LANG_BG_IC_ALLIANCE : LANG_BG_IC_HORDE));
...
void BattlegroundIC::DestroyGate(Player* pl, GameObject* go, uint32 /*destroyedEvent*/)
{
    ...
    DoorOpen((pl->GetBGTeam() == ALLIANCE ? BG_IC_GO_HORDE_KEEP_PORTCULLIS : BG_IC_GO_DOODAD_PORTCULLISACTIVE02));
    ...
    SendMessage2ToAll(lang_entry, CHAT_MSG_BG_SYSTEM_NEUTRAL, NULL, (pl->GetBGTeam() == ALLIANCE ? LANG_BG_IC_HORDE_KEEP : LANG_BG_IC_ALLIANCE_KEEP));
}
