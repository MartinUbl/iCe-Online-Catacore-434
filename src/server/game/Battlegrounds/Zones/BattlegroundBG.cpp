...
BattlegroundBG::BattlegroundBG()
{
    ...
    m_BgCreatures.resize(BG_BG_ALL_NODES_COUNT + BG_BG_DYNAMIC_NODES_COUNT);//+3 for aura triggers
...
void BattlegroundBG::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (m_TeamScores[BG_TEAM_ALLIANCE] >= BG_BG_MAX_TEAM_SCORE)
            winner = ALLIANCE;
        else if (m_TeamScores[BG_TEAM_HORDE] >= BG_BG_MAX_TEAM_SCORE)
            winner = HORDE;
        ...
        if (winner)
        {
            RewardHonorToTeam(BG_BG_Honor[BG_BG_WIN], winner);
...
int32 BattlegroundBG::_GetNodeNameId(uint8 node)
{
    switch (node)
    {
        ...
        default:
            //ASSERT(0);
            // I think it's not a good idea to make program crash only because of node id
            // out of range (cosmetic thing mostly - function conditions in other places)
            break;
    }
    return 0;
}
...
void BattlegroundBG::_NodeOccupied(uint8 node,Team team)
{
    ...
    if (!trigger)
       trigger = AddCreature(WORLD_TRIGGER,node+BG_BG_ALL_NODES_COUNT,team,BG_BG_NodePositions[node][0],BG_BG_NodePositions[node][1],BG_BG_NodePositions[node][2],BG_BG_NodePositions[node][3]);
...
void BattlegroundBG::_NodeDeOccupied(uint8 node)
{
    ...
    if(node < BG_BG_DYNAMIC_NODES_COUNT)//only dynamic nodes, no start points
        DelCreature(node+BG_BG_ALL_NODES_COUNT);//NULL checks are in DelCreature! 0-4 spirit guides
...
void BattlegroundBG::Reset()
{
    ...
    for (uint8 i = 0; i < BG_BG_ALL_NODES_COUNT + BG_BG_DYNAMIC_NODES_COUNT; ++i)//+3 for aura triggers
        if (m_BgCreatures[i])
            DelCreature(i);
}
