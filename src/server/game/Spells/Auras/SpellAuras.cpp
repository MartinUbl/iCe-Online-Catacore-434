...
{
    return TryCreate(spellproto, MAX_EFFECT_MASK, owner, caster, baseAmount, scriptedAmount, castItem, casterGUID);
}
    ...
    if (apply)
    {
        ...
        switch (GetSpellProto()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellProto()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    if (AuraEffect const * aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 0))
                    {
                        ...
                        int32 heal = (aurEff->GetAmount() * basepoints0) / 100;
            ...
            case SPELLFAMILY_PALADIN: // Speed of Light (talent)
                if(GetId() == 498)
                {
                    // Amount of speed is hidden in second dummy effect
                    if (AuraEffect * aurEff = target->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_PALADIN, 5062, EFFECT_1))
                    {
                        int32 bp0 = aurEff->GetAmount();
                        target->CastCustomSpell(target, 85497, &bp0, 0, 0, true);
                    }
                }
                // Inquisition
                else if(GetId() == 84963)
