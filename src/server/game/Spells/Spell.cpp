...
void Spell::SendSpellGo()
{
    ...
    */
    ...
    if (castFlags & CAST_FLAG_RUNE_LIST)                   // rune cooldowns list
    {
        ...
        {
            maxcooldown = playerCaster->GetRuneMaxCooldown(i);
            if (maxcooldown == 0)
                maxcooldown = RUNE_COOLDOWN;

            data << uint8(255 - 255*playerCaster->GetRuneCooldown(i)/maxcooldown); // remaining cooldown time
        }
    }
