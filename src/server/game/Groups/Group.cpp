...
void Group::SendLootRoll(const uint64& SourceGuid, const uint64& TargetGuid, uint32 RollNumber, uint8 RollType, const Roll &r)
{
    ...
    data << uint32(r.itemRandomSuffix);                     // randomSuffix
    data << uint32(r.itemRandomPropId);                     // Item random property ID
    data << uint32(RollNumber);                             // 0: "Need for: [item name]" >= 0x80000000: don't show roll number for other types than need
...
void Group::SendLootRollWon(const uint64& SourceGuid, const uint64& TargetGuid, uint8 RollNumber, uint8 RollType, const Roll &r)
{
    ...
    data << uint64(SourceGuid);                             // guid of the item rolled
    data << uint32(r.itemSlot);                             // unknown, maybe amount of players
    data << uint32(r.itemid);                               // the itemEntryId for the item that shall be rolled for
    data << uint32(r.itemRandomSuffix);                     // randomSuffix
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            if (r->totalPlayersRolling > 0)
            {
                r->setLoot(loot);
                r->itemSlot = itemSlot;
                if (item->DisenchantID && int32(m_maxEnchantingLevel) >= item->RequiredDisenchantSkill)
...
void Group::CountRollVote(const uint64& playerGUID, const uint64& Guid, uint32 NumberOfPlayers, uint8 Choice)
{
    ...
    switch (Choice)
    {
        case ROLL_PASS:                                     // Player choose pass
            SendLootRoll(0, playerGUID, 0x80000000, ROLL_PASS, *roll);
            ++roll->totalPass;
            itr->second = PASS;
            break;
        ...
        case ROLL_GREED:                                    // player choose Greed
            SendLootRoll(0, playerGUID, 0x80000000, ROLL_GREED, *roll);
            ++roll->totalGreed;
            itr->second = GREED;
            break;
        case ROLL_DISENCHANT:                               // player choose Disenchant
            SendLootRoll(0, playerGUID, 0x80000000, ROLL_DISENCHANT, *roll);
            ++roll->totalGreed;
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const* bgOrTemplate, BattlegroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/, bool isRated, uint32 arenaSlot, bool isWarGame)
{
    ...
    for (GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next(), ++memberscount)
    {
        ...
        if (bgOrTemplate->GetTypeID() != BATTLEGROUND_AA && !member->CanJoinToBattleground() && !isWarGame)
        ...
        if (member->GetTwinkType() != twinkType && !isWarGame)
    ...
    return (bgOrTemplate->GetTypeID())?ERR_BATTLEGROUND_NONE:ERR_BATTLEGROUND_JOIN_FAILED;
}
...
void Group::RefreshMarkerBySpellIdToPlayer(Player * p,uint32 spellId)
{
    Player * rl = this->GetLeader();

    if (p == NULL || p->IsInWorld() == false || rl == NULL || !rl->IsInWorld())
        return;
    ...
    if (Markers && !Markers->empty())
    {
        ...
        for (Player::GUIDTimestampMap::iterator itr = Markers->begin(); itr != Markers->end();++itr)
        {
            ...
            if (pTemp && pTemp->IsInWorld() && pTemp->GetUInt32Value(UNIT_CREATED_BY_SPELL) == spellId)
            {
                ...
                p->GetSession()->SendPacket(&visual);
                break;
            }
        }
    }
...
void Group::RefreshMarkerBySpellIdToGroup(uint32 spellId)
{
    Player * rl = this->GetLeader();
    ...
    if (Markers && !Markers->empty())
    {
        ...
        for (Player::GUIDTimestampMap::iterator itr = Markers->begin(); itr != Markers->end();++itr)
        {
            ...
            if (pTemp && pTemp->IsInWorld() && pTemp->GetUInt32Value(UNIT_CREATED_BY_SPELL) == spellId)
            {
                ...
                WorldPacket visual;
                BuildMarkerVisualPacket(&visual, pTemp->GetGUID(), pos);
                //rl->GetSession()->SendPacket(&visual);
                BroadcastPacket(&visual, false);
                break;
            }
        }
    }
...
void Group::RemoveMarkerBySpellId(Player * rL,uint32 spellId)
{
    if (!rL || !rL->IsInWorld() || !spellId)
        return;
