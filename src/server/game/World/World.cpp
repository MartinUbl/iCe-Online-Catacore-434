...
void World::LoadConfigSettings(bool reload)
{
    ...
    rate_values[RATE_XP_GATHER]   = sConfig->GetFloatDefault("Rate.XP.Gather", 1.0f);
    ...
    else if (m_int_configs[CONFIG_START_PLAYER_MONEY] > 0x7fffffff)
    {
        sLog->outError("StartPlayerMoney (%i) must be in range 0..%u. Set to %u.",
            m_int_configs[CONFIG_START_PLAYER_MONEY], 0x7fffffff, 0x7fffffff);
        m_int_configs[CONFIG_START_PLAYER_MONEY] = 0x7fffffff;
    }
    ...
    m_bool_configs[CONFIG_ENABLE_MMAPS] = sConfig->GetBoolDefault("mmap.enablePathFinding", false);
    ...
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MAX] = sConfig->GetIntDefault("Wintergrasp.PlayerMax", 100);
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MIN] = sConfig->GetIntDefault("Wintergrasp.PlayerMin", 0);
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MIN_LVL] = sConfig->GetIntDefault("Wintergrasp.PlayerMinLvl", 77);
...
void World::SendWorldTextToChannel(std::string channelName,int32 string_id, ...)
{
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if (ChannelMgr* cMgr = channelMgr(player->GetTeam()))
            ...
            if (channel->IsPlayerInChannel(player->GetGUID()) && !(channelName.compare("krcma") == 0 && player->isDisabledPVPAnnounceStatus()))  
                    wt_do(itr->second->GetPlayer());
    }
