...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss, bool splitDmaage)
{
    ...
    // Ignore splited damage from SPELL_AURA_SPLIT_DAMAGE_PCT
    if (!splitDmaage && cleanDamage && damagetype == DIRECT_DAMAGE && this != pVictim && getPowerType() == POWER_RAGE)
    {
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 53557:  // Enlightened Judgements (Rank 2)
            if (procSpell->Id != 54158 && procSpell->Id != 31804 && procSpell->Id != 20187) // Proc only from Judgements
                return false;
            break;
...
bool Unit::Attack(Unit *victim, bool meleeAttack)
{
    ...
    if (!IsInWorld() || !IsAlive() || !victim->IsInWorld() || !victim->IsAlive())
        return false;
...
void Unit::ClearInCombat()
{
    ...
    else
    {
        ...
        if (Aura* aur = ToPlayer()->GetAura(50034)) // Blood Rites
            aur->GetEffect(0)->ResetPeriodic(true);
        if (Aura* aur = ToPlayer()->GetAura(56835)) // Reaping
            aur->GetEffect(0)->ResetPeriodic(true);
...
void Unit::SendTeleportPacket(Position &pos)
{
    ...
    data.WriteByteSeq(guid[3]);
    data.WriteByteSeq(guid[5]);
    data << float(pos.m_positionX);
    data.WriteByteSeq(guid[4]);
    data << float(pos.m_orientation);
    data.WriteByteSeq(guid[7]);
    data << float(pos.m_positionZ);
    data.WriteByteSeq(guid[0]);
    data.WriteByteSeq(guid[6]);
    data << float(pos.m_positionY);
