...
enum SpellInterruptFlags
{
    ...
    SPELL_INTERRUPT_FLAG_AUTOATTACK   = 0x04, // not tested, probably wrong
...
enum NPCFlags
{
    ...
    UNIT_NPC_FLAG_MAILBOX               = 0x04000000,       // creature can serve as mailbox
    UNIT_NPC_FLAG_REFORGER              = 0x08000000,       // reforging
    UNIT_NPC_FLAG_TRANSMOGRIFIER        = 0x10000000,       // transmogrification
    UNIT_NPC_FLAG_VAULTKEEPER           = 0x20000000,       // void storage
    UNIT_NPC_FLAG_GUARD                 = 0x40000000,       // custom flag for guards
    UNIT_NPC_FLAG_OUTDOORPVP            = 0x80000000,       // custom flag for outdoor pvp creatures
};
...
class Unit : public WorldObject
{
    public:
        ...
        void BuildPlaySpellVisualKit(WorldPacket* data, uint32 id, uint32 unkParam = 0, uint32 unkParam2 = 0);
        void SendPlaySpellVisualKit(uint32 id, uint32 unkParam = 0, uint32 unkParam2 = 0);
        void SendPlaySpellVisualKitFactioned(uint32 id_friendly, uint32 id_hostile);
        ...
        uint32 DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage = NULL, DamageEffectType damagetype = DIRECT_DAMAGE, SpellSchoolMask damageSchoolMask = SPELL_SCHOOL_MASK_NORMAL, SpellEntry const *spellProto = NULL, bool durabilityLoss = true, bool splitDmaage = false);
        ...
        float GetUnitCriticalChance(WeaponAttackType attackType, const Unit *pVictim, SpellEntry const *spell = NULL) const;
        ...
        bool IsReforger()     const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_REFORGER); }
        bool IsTransmogrifier() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TRANSMOGRIFIER); }
        bool IsVoidKeeper() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VAULTKEEPER); }
        bool IsServiceProvider() const
        {
            return HasFlag(UNIT_NPC_FLAGS,
                ...
                UNIT_NPC_FLAG_REFORGER | UNIT_NPC_FLAG_TRANSMOGRIFIER | UNIT_NPC_FLAG_VAULTKEEPER);
        }
        ...
        bool isAttackableByAOE(bool requireDeadTarget = false, SpellEntry const *spellProto = NULL) const;
        ...
        void CastCustomSpell(uint32 spellId, CustomSpellValues const &value, Position &pos, bool triggered = true, Item *castItem = NULL, AuraEffect const * triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void RemovePlayerAurasWithSameAuraTypeDueToStack(AuraType aurType, AuraEffect *aurEff, uint64 casterGUID, uint32 exceptSpellId);
        ...
        uint32 SpellDamageBonusDone(Unit* victim, SpellEntry const* spellProto, uint32 effIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack = 1) const;
        float  SpellDamagePctDone(Unit* victim, SpellEntry const* spellProto, DamageEffectType damagetype) const;
        uint32 SpellDamageBonusTaken(Unit* caster, SpellEntry const* spellProto, uint32 effIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack = 1) const;
        ...
        uint32 SpellHealingBonusDone(Unit* victim, SpellEntry const* spellProto, uint32 effIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack = 1) const;
        float SpellHealingPctDone(Unit* victim, SpellEntry const* spellProto) const;
        uint32 SpellHealingBonusTaken(Unit* caster, SpellEntry const* spellProto, uint32 effIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack = 1) const;

        void MeleeDamageBonus(Unit *pVictim, uint32 *damage, WeaponAttackType attType, SpellEntry const *spellProto = NULL);
        uint32 MeleeDamageBonusDone(Unit* pVictim, uint32 damage, WeaponAttackType attType, SpellEntry const* spellProto = NULL);
        uint32 MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellEntry const* spellProto = NULL);
        ...
        uint32 GetRemainingPeriodicAmount(uint64 caster, uint32 spellId, AuraType auraType, uint8 effectIndex) const;
        ...
        uint32 CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, bool exact = false) const;
        uint32 GetAbsorbedDamageFromSpell(Unit* victim, SpellEntry const* spellInfo) const;
        ...
        bool CanReachByPath(Unit const* target) const;
        ...
        void RewardRage(uint32 damage, float weaponSpeedHitFactor, bool attacker, DamageEffectType damageType = DIRECT_DAMAGE);
    ...
    private:
        ...
        void HandleProcTriggerSpellCopy(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlag);
...
//        ThreatManager m_ThreatManager;
        ...
        struct DamageTakenRecord { uint32 timestamp, damage; };
        std::list<DamageTakenRecord> m_lDamageTakenHistory;
