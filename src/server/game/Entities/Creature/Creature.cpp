...
bool Creature::IsImmunedToSpell(SpellEntry const* spellInfo, uint32 effectMask)
{
    if (!spellInfo)
        return false;
...
bool Creature::_IsTargetAcceptable(const Unit *target) const
{
    ...
    if (IsFriendlyTo(target)
        ...
        || target->HasUnitState(UNIT_STATE_DIED)
...
{
    m_CreatureCategoryCooldowns[category] = m_LogonTimer + msDuration;
}
...
void Creature::AddCreatureSpellCooldown(uint32 spellid)
{
    ...
    if (cooldown)
        _AddCreatureSpellCooldown(spellid, cooldown);

    if (spellInfo->Category)
        _AddCreatureCategoryCooldown(spellInfo->Category, spellInfo->CategoryRecoveryTime);
...
bool Creature::HasCategoryCooldown(uint32 spell_id) const
{
    ...
    return(itr != m_CreatureCategoryCooldowns.end() && itr->second > m_LogonTimer);
}

bool Creature::HasSpellCooldown(uint32 spell_id) const
{
    CreatureSpellCooldowns::const_iterator itr = m_CreatureSpellCooldowns.find(spell_id);
    return (itr != m_CreatureSpellCooldowns.end() && itr->second > m_LogonTimer) || HasCategoryCooldown(spell_id);
}
