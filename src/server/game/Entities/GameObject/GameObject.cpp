...
void GameObject::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        bool toggledState = (GetGOData() ? (GetGOData()->go_state == GO_STATE_READY) : false) || IsTransport();
        if (m_model)
...
void GameObject::SetTransportState(GOState state, uint32 stopFrame /*= 0*/, uint32 stateChangeTime /*= 0*/)
{
