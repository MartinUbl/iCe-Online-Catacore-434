...
void WorldSession::HandleObjectUpdateFailed(WorldPacket& recv_data)
{
    ...
    sLog->outDebug("Object update failed for object guid %u (" UI64FMTD ") (%s), player %s", GUID_LOPART(uint64(guid)), uint64(guid), obj?obj->GetName():"not-found", GetPlayer()->GetName());
}
