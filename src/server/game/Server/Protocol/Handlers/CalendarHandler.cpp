...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_GET_CALENDAR [" UI64FMTD "]", guid);
    ...
    for (CalendarEventStore::const_iterator itr = playerEvents.begin(); itr != playerEvents.end(); ++itr)
    {
        ...
        Guild* guild = sObjectMgr->GetGuildById(calendarEvent->GetGuildId());
...
void WorldSession::HandleCalendarGetEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_GET_EVENT. Player ["
...
{
    sLog->outDebug("CMSG_CALENDAR_GUILD_FILTER [" UI64FMTD "]", _player->GetGUID());
    ...
    sLog->outDebug("CMSG_CALENDAR_GUILD_FILTER: Min level [%d], Max level [%d], Min rank [%d]", minLevel, maxLevel, minRank);
}

void WorldSession::HandleCalendarArenaTeam(WorldPacket& recvData)
{
    sLog->outDebug("CMSG_CALENDAR_ARENA_TEAM [" UI64FMTD "]", _player->GetGUID());

    uint32 arenaTeamId;
    recvData >> arenaTeamId;

    if (ArenaTeam* team = sObjectMgr->GetArenaTeamById(arenaTeamId))
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_UPDATE_EVENT [" UI64FMTD "] EventId [" UI64FMTD
...
void WorldSession::HandleCalendarCopyEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_COPY_EVENT [" UI64FMTD "], EventId [" UI64FMTD
...
{
    sLog->outDebug("CMSG_CALENDAR_EVENT_INVITE");
    ...
    // TODO : Implement CONFIG_ALLOW_TWO_SIDE_INTERACTION_CALENDAR if needed in future
...
void WorldSession::HandleCalendarEventSignup(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_EVENT_SIGNUP [" UI64FMTD "] EventId [" UI64FMTD "] Tentative %u", guid, eventId, tentative);
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_EVENT_RSVP [" UI64FMTD "] EventId ["
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_EVENT_REMOVE_INVITE ["
...
void WorldSession::HandleCalendarEventStatus(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_EVENT_STATUS [" UI64FMTD"] EventId ["
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_EVENT_MODERATOR_STATUS [" UI64FMTD "] EventId ["
        UI64FMTD "] ownerInviteId [" UI64FMTD "], Invitee ([" UI64FMTD "] id: ["
...
void WorldSession::HandleCalendarComplain(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_COMPLAIN [" UI64FMTD "] EventId ["
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket& /*recvData*/)
{
    ...
    sLog->outDebug("CMSG_CALENDAR_GET_NUM_PENDING: [" UI64FMTD
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& recvData)
{
    ...
    sLog->outDebug("CMSG_SET_SAVED_INSTANCE_EXTEND - MapId: %u, Difficulty: %u, ToggleExtend: %s", mapId, difficulty, toggleExtend ? "On" : "Off");
...
{
    if (add)
        sLog->outDebug("SMSG_CALENDAR_RAID_LOCKOUT_ADDED");
    else
        sLog->outDebug("SMSG_CALENDAR_RAID_LOCKOUT_REMOVED");
    time_t currTime = time(NULL);
...
void WorldSession::SendCalendarRaidLockoutUpdated(InstanceSave const* save)
{
    ...
    uint64 guid = _player->GetGUID();
    sLog->outDebug("SMSG_CALENDAR_RAID_LOCKOUT_UPDATED [" UI64FMTD
