...
void WorldSession::HandleGuildFinderBrowse(WorldPacket& recvPacket)
{
    ...
    if (!(classRoles & GUILDFINDER_ALL_ROLES) || classRoles > GUILDFINDER_ALL_ROLES ||
        !(availability & AVAILABILITY_ALWAYS) || availability > AVAILABILITY_ALWAYS ||
        !(guildInterests & ALL_INTERESTS) || guildInterests > ALL_INTERESTS ||
        playerLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL) || playerLevel < 1)
    {
        // an update packet has to be sent due to "guild finder freeze"
    ...
    for (LFGuildStore::const_iterator itr = guildList.begin(); itr != guildList.end(); ++itr)
    {
        ...
        Guild* guild = sObjectMgr->GetGuildById(itr->first);
        ...
        bufferData << uint32(0);//uint32(guild->GetAchievementMgr().GetAchievementPoints());
...
void WorldSession::HandleGuildFinderGetApplications(WorldPacket& /*recvPacket*/)
{
    ...
    if (applicationsCount > 0)
    {
        ...
        {
            Guild* guild = sObjectMgr->GetGuildById(itr->GetGuildId());
...
void WorldSession::HandleGuildFinderSetGuildPost(WorldPacket& recvPacket)
{
    ...
    if (Guild* guild = sObjectMgr->GetGuildById(player->GetGuildId())) // Player must be guild master
        if (guild->GetLeaderGUID() != player->GetGUID())
            return;
