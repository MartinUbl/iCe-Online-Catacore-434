...
{
    sLog->outDebug("WORLD: Received CMSG_VOID_STORAGE_UNLOCK");
    ...
    if (!unit)
    {
        sLog->outDebug("WORLD: HandleVoidStorageUnlock - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }

    if (player->IsVoidStorageUnlocked())
    {
        sLog->outDebug("WORLD: HandleVoidStorageUnlock - Player (GUID: %u, name: %s) tried to unlock void storage a 2nd time.", player->GetGUIDLow(), player->GetName());
        return;
    }
...
{
    sLog->outDebug( "WORLD: Received CMSG_VOID_STORAGE_QUERY");
    Player* player = GetPlayer();
    ...
    if (!unit)
    {
        sLog->outDebug("WORLD: HandleVoidStorageQuery - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }

    if (!player->IsVoidStorageUnlocked())
    {
        sLog->outDebug("WORLD: HandleVoidStorageQuery - Player (GUID: %u, name: %s) queried void storage without unlocking it.", player->GetGUIDLow(), player->GetName());
        return;
    }
    ...
    for (uint8 i = 0; i < VOID_STORAGE_MAX_SLOT; ++i)
    {
        ...
        itemData << uint32(item->ItemSuffixFactor); // unk, SuffixFactor? large ints, both positive and negative appear here
...
{
    sLog->outDebug("WORLD: Received CMSG_VOID_STORAGE_TRANSFER");
    Player* player = GetPlayer();
    ...
    if (countDeposit > 9)
    {
        sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) wants to deposit more than 9 items (%u).", player->GetGUIDLow(), player->GetName(), countDeposit);
        return;
    }
    ...
    if (countWithdraw > 9)
    {
        sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) wants to withdraw more than 9 items (%u).", player->GetGUIDLow(), player->GetName(), countWithdraw);
        return;
    }
    ...
    if (!unit)
    {
        sLog->outDebug("WORLD: HandleVoidStorageTransfer - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }

    if (!player->IsVoidStorageUnlocked())
    {
        sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) queried void storage without unlocking it.", player->GetGUIDLow(), player->GetName());
        return;
    }
    ...
    for (std::vector<ObjectGuid>::iterator itr = itemGuids.begin(); itr != itemGuids.end(); ++itr)
    {
        ...
        if (!item)
        {
            sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) wants to deposit an invalid item (item guid: " UI64FMTD ").", player->GetGUIDLow(), player->GetName(), (uint64)*itr);
            continue;
        }
    ...
    for (std::vector<ObjectGuid>::iterator itr = itemIds.begin(); itr != itemIds.end(); ++itr)
    {
        ...
        if (!itemVS)
        {
            sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) tried to withdraw an invalid item (id: " UI64FMTD ")", player->GetGUIDLow(), player->GetName(), (uint64)*itr);
            continue;
        }

        ItemPosCountVec dest;
        uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemVS->ItemEntry, 1);
        if (msg != EQUIP_ERR_OK)
        {
            SendVoidStorageTransferResult(VOID_TRANSFER_ERROR_INVENTORY_FULL);
            sLog->outDebug("WORLD: HandleVoidStorageTransfer - Player (GUID: %u, name: %s) couldn't withdraw item id " UI64FMTD " because inventory was full.", player->GetGUIDLow(), player->GetName(), (uint64)*itr);
            return;
        }
...
{
    sLog->outDebug("WORLD: Received CMSG_VOID_SWAP_ITEM");
    ...
    if (!unit)
    {
        sLog->outDebug("WORLD: HandleVoidSwapItem - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }

    if (!player->IsVoidStorageUnlocked())
    {
        sLog->outDebug("WORLD: HandleVoidSwapItem - Player (GUID: %u, name: %s) queried void storage without unlocking it.", player->GetGUIDLow(), player->GetName());
        return;
    }
    ...
    {
        sLog->outDebug("WORLD: HandleVoidSwapItem - Player (GUID: %u, name: %s) requested swapping an invalid item (slot: %u, itemid: " UI64FMTD ").", player->GetGUIDLow(), player->GetName(), newSlot, (uint64)itemId);
        return;
    }
