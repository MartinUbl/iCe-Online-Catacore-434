...
void WorldSession::HandlePetAction(WorldPacket & recv_data)
{
    ...
    sLog->outDebug("HandlePetAction: Pet %u - flag: %u, spellid: %u, target: %u.", uint32(GUID_LOPART(guid1)), uint32(flag), spellid, uint32(GUID_LOPART(guid2)));
...
void WorldSession::HandlePetActionHelper(Unit *pet, uint64 guid1, uint32 spellid, uint16 flag, uint64 guid2, float pos_x, float pos_y, float pos_z)
{
    ...
    switch(flag)
    {
        ...
        case ACT_REACTION:                                  // 0x6
            switch(spellid)
            {
                case REACT_PASSIVE:                         // passive means immediate attack stop
                    pet->AttackStop();

                case REACT_DEFENSIVE:                       // defensive
                case REACT_AGGRESSIVE:                      // aggresive (no longer available, probably)
                case REACT_ASSIST:                          // assist stance
