...
void CalendarMgr::LoadFromDB()
{
    ...
    sLog->outString("server.loading", ">> Loaded %u calendar events", count);
    count = 0;
    ...
    sLog->outString("server.loading", ">> Loaded %u calendar invites", count);
...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    sLog->outDebug("calendar", "CalendarMgr::GetEvent: [" UI64FMTD "] not found!", eventId);
    return NULL;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    sLog->outDebug("calendar", "CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
    return NULL;
}
...
void CalendarMgr::SendCalendarEventInviteAlert(CalendarEvent const& calendarEvent, CalendarInvite const& invite)
{
    ...
    {
        if (Guild* guild = sObjectMgr->GetGuildById(calendarEvent.GetGuildId()))
