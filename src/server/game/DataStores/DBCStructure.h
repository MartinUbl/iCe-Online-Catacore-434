...
struct AchievementCriteriaEntry
{
    ...
    union
    {
        ...
        // ACHIEVEMENT_CRITERIA_TYPE_EARN_CURRENCY_TYPE       = 12
        struct
        {
            uint32  currencyId;                             // 3
            uint32  count;                                  // 4
        ...
        // ACHIEVEMENT_CRITERIA_TYPE_REACH_PERSONAL_RATING  = 39
        struct
        {
            uint32  teamtype;                               // 3 {2,3,5}
            uint32  personalrating;                         // 4
        } reach_personal_rating;
...
// Probably changed structure, but who knows?
/*
    //uint32 unk1; // 5,    4.3.4, all zeros

    struct
    {
        uint32  additionalRequirement_type;                 // 6, 8
        uint32  additionalRequirement_value;                // 7, 9
    } additionalRequirements[MAX_CRITERIA_REQUIREMENTS];

    DBCString  name;                                        // 10
    uint32  completionFlag;                                 // 11
    uint32  timedType;                                      // 12 Only appears with timed achievements, seems to be the type of starting a timed Achievement, only type 1 and some of type 6 need manual starting
                                                                  // 1: ByEventId(?) (serverside IDs),    2: ByQuestId,   5: ByCastSpellId(?)
                                                                  // 6: BySpellIdTarget(some of these are unknown spells, some not, some maybe spells)
                                                                  // 7: ByKillNpcId,  9: ByUseItemId
...
struct BattlemasterListEntry
{
    ...
    //DBCString unk1;                                       // 10 (unk, on bgs and arenas its always 'Alterac Valley')
    DBCString name;                                         // 11
...
struct EmotesEntry
{
    ...
    //uint32  unk;                                          // 7, unk 4.3.4, present with STATE_READ emotes
};
...
struct LFGDungeonEntry
{
    ...
    //uint32  unk4;                                         // 14
    uint32  grouptype;                                      // 15
    //DBCString   desc;                                     // 16 Description
    //uint32 unk1;                                          // 17 4.3.4
    //uint32 unk2;                                          // 18 4.3.4, 0 = raid, 1 = dungeon, 2 = ??
    //uint32 unk3;                                          // 19 4.3.4, 0 = raid, 1 = dungeon, 6 = ??
    //uint32 unk4;                                          // 20 4.3.4, 0 = raid, 3 = dungeon, 17 = ??
...
struct ScalingStatDistributionEntry
{
    ...
    uint32  MinLevel;                                       // 21 players below this can't equip the item (relevant only for id 393 as of patch 4.3.4)
    uint32  MaxLevel;                                       // 22
};
...
struct SkillLineAbilityEntry
{
    ...
    uint32    grandSkillup;                                 // 12       how much skills does this give
    //uint32                                                // 13       4.0.0
};
...
struct SpellEffectEntry
{
    ...
    uint32    EffectFlags;                                  // 24       new 4.3.4
};
...
struct SpellPowerEntry
{
    ...
    //uint32  unk1;                                         // 6        4.0.0
    //float   unk2;                                         // 7        4.3.4, percentage, unknown purpose
};
...
struct SpellEntry_n //Structure of the true Spell.dbc 
{
    ...
    uint32    AttributesEx9;                                // 10       4.0.0, attributes?
    uint32    AttributesEx10;                               // 11       4.3.4, attributes?
    ...
    //DBCString SpellNote;                                  // 22       m_spell_note not used (Rank 1, Special Ability,..)
    ...
    float  ap_bonus;                                        // 30       attack power bonus
...
struct SpellShapeshiftFormEntry
{
    ...
    uint32 mountTypeIndex;                                  // 19 mount type index (MountType.dbc)
    //uint32 unk6;                                          // 20
};
...
struct TaxiNodesEntry
{
    ...
    uint32    team_id;                                      // 8        4.3.4, something to do with team which can use this node?
    //float     unk1;                                       // 9        4.3.4
    //float     unk2;                                       // 10       4.3.4
};
...
struct VehicleSeatEntry
{
    ...
    bool IsUsableByAura() const { return m_flagsB & (VEHICLE_SEAT_FLAG_B_USABLE_FORCED | VEHICLE_SEAT_FLAG_B_USABLE_FORCED_2 | VEHICLE_SEAT_FLAG_B_USABLE_FORCED_3 | VEHICLE_SEAT_FLAG_B_USABLE_FORCED_4); }
};
