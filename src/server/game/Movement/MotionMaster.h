...
class MotionMaster //: private std::stack<MovementGenerator *>
{
    ...
    public:
        ...
        explicit MotionMaster(Unit *unit) : i_top(-1), i_owner(unit), m_expList(NULL), m_cleanFlag(MMCF_NONE), m_pathfindingState(0)
        {
        ...
        void MovePoint(uint32 id, const Position &pos, bool generatePath = false, bool clearPrevious = true)
            { MovePoint(id, pos.m_positionX, pos.m_positionY, pos.m_positionZ,generatePath,clearPrevious); }
        void MovePoint(uint32 id, float x, float y, float z, bool generatePath = false, bool clearPrevious = true);
