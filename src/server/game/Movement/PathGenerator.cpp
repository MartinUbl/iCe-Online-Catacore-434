...
_navMeshQuery(NULL)
{
    ...
    sLog->outDebug("++ PathGenerator::PathGenerator for %u \n", _sourceUnit->GetGUIDLow());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    if (_forceSource)
        SetStartPosition(_forcedStartPosition);
    else
    ...
    sLog->outDebug("++ PathGenerator::CalculatePath() for %u \n", _sourceUnit->GetGUIDLow());

    // make sure navMesh works - we can run on map w/o mmap
    // check if the start and end point have a .mmtile loaded (can we pass via not loaded tile on the way?)
    if (!_navMesh || !_navMeshQuery || _sourceUnit->hasUnitState(UNIT_STAT_IGNORE_PATHFINDING) ||
...
void PathGenerator::SetForceSource(float x, float y, float z)
{
    _forceSource = true;

    _forcedStartPosition.x = x;
    _forcedStartPosition.y = y;
    _forcedStartPosition.z = z;
}
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                sLog->outDebug("Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    sLog->outDebug("++ PathGenerator::BuildPointPath path type %d size %d poly-size %d\n", _type, pointCount, _polyLength);
}
